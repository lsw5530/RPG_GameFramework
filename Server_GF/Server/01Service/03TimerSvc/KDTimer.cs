...
public class KDTimer {
    ...
    private List<KDTimeTask> tmpTimeLst = new List<KDTimeTask>();
    private List<KDTimeTask> taskTimeLst = new List<KDTimeTask>();
    private List<int> tmpDelTimeLst = new List<int>();

    private int frameCounter;

    private static readonly string lockFrame = "lockFrame";
    private List<KDFrameTask> tmpFrameLst = new List<KDFrameTask>();
    private List<KDFrameTask> taskFrameLst = new List<KDFrameTask>();
    private List<int> tmpDelFrameLst = new List<int>();

    public KDTimer(int interval = 0) {
        tidLst.Clear();
        recTidLst.Clear();
            ...
            lock (lockFrame) {
                for (int i = 0; i < tmpDelFrameLst.Count; i++) {
                    ...
                    for (int j = 0; j < taskFrameLst.Count; j++) {
                        KDFrameTask task = taskFrameLst[j];
                        if (task.tid == delTid) {
                            isDel = true;
                    ...
                    for (int j = 0; j < tmpFrameLst.Count; j++) {
                        KDFrameTask task = tmpFrameLst[j];
                        if (task.tid == delTid) {
    ...
    private void CheckTimeTask() {
        ...
        for (int index = 0; index < taskTimeLst.Count; index++) {
            KDTimeTask task = taskTimeLst[index];
    ...
    private void CheckFrameTask() {
        ...
        for (int index = 0; index < taskFrameLst.Count; index++) {
            KDFrameTask task = taskFrameLst[index];
    ...
    public int AddFrameTask(Action<int> callback, int delay, int count = 1) {
        int tid = GetTid();
        lock (lockTime) {
            tmpFrameLst.Add(new KDFrameTask(tid, callback, frameCounter + delay, delay, count));
        }
        return tid;
    }
    ...
    public bool ReplaceFrameTask(int tid, Action<int> callback, int delay, int count = 1) {
        KDFrameTask newTask = new KDFrameTask(tid, callback, frameCounter + delay, delay, count);
    ...
    class KDTimeTask {
        ...
        public KDTimeTask(int tid, Action<int> callback, double destTime, double delay, int count) {
            this.tid = tid;
    ...
    class KDFrameTask {
        ...
        public KDFrameTask(int tid, Action<int> callback, int destFrame, int delay, int count) {
            this.tid = tid;
            this.callback = callback;
