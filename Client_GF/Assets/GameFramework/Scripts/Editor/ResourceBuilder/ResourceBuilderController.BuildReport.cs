...
namespace UnityGameFramework.Editor.ResourceTools {
    ...
    public sealed partial class ResourceBuilderController
    {
        private sealed class BuildReport
        {
            ...
            internal void LogVersionInfo(string buildReportPath, string platformName, string internalGameVersion,
               int internalResourceVersion, string path, int length, int hashCode, int zipLength, int zipHashCode) {
                VersionInfo vi = new VersionInfo();
                vi.ForceGameUpdate = false;
                vi.LatestGameVersion = "0.1.0";
                vi.InternalGameVersion = int.Parse(internalGameVersion.Split('.')[0]);
                vi.InternalResourceVersion = internalResourceVersion;
                vi.GameUpdateUrl = "http://127.0.0.1:8080/" + internalGameVersion.Replace(".", "_") + "_" + internalResourceVersion;
                vi.VersionListLength = length;
                vi.VersionListHashCode = hashCode;
                vi.VersionListZipLength = zipLength;
                vi.VersionListZipHashCode = zipHashCode;

                string versionName = "version_" + platformName + ".txt";
                string outPath = Utility.Path.GetRegularPath(Path.Combine(buildReportPath, versionName));

                File.WriteAllText(outPath, JsonMapper.ToJson(vi));
            }
        }
    }
}
